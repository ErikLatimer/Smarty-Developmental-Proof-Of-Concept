 ...
 * IMPORTANT
 * 
 * THIS FILE SHOULD BE NAMED LIGHSTREAMER REMOTE DATA ADAPTER. NO NADEX FUNCTIONALITY HAS BEEN ADDED YET.
 * 
 * THIS INCLUDES BOTH THE DATA ADAPTER AND THE METAD DATA ADAPTER. REMEMBER THAT THIS FILE HAS 
 * TO RUN IN ORDER TO BIND TO THE PORTS OF THE LIGHTSTREAMER SERVICES ASKING FOR THE DATA ADAPTER
 * AND METADATA ADAPTER, AND ISSUES COMMANDS OVER THESE PORT CONNECTIONS THAT WE EXCUTE AND FULLFILL 
 * IN THIS FILE.
 * 
 * 
 * 
...
const REQUIREDPROPERTIESPAUSEANDRESUME = {
    'UID': undefined,
}
var requiredkeys;
const REQUIREDCOMMANDPROPERTY = 'command';
const AVAILABLECOMMANDS = [ 'CREATE', 'PAUSE', 'RESUME' ];
...
METADATAPROVIDER.on('notifyUserMessage', function (request, response) {
    ...
    /*
        The PAUSE command should look like this in JSON
    ...
    */
   // COMMAND VERIFIED TO WORK!!!
    else if (json.command == 'PAUSE') {
        requiredkeys = Object.keys(REQUIREDPROPERTIESPAUSEANDRESUME);
        requiredkeys.forEach((key) => {
            if (!(keysInObject.includes(key))) {
                var error_message = ("ERROR WITHIN METADATAPROVIDER.on( notifyUserMessage, ... , the " + key + " property of the JSON message sent by the user is either not present.");
                console.error(error_message + "\nReturning and calling response.error()...");
        ...
        customStreamItemMap.get(json[requiredkeys[0]]).paused = false;
        response.success();
    }
...
function customStream(customStreamItemObject) {
    ...
    // We want to use startTimeStamp when its the first time we search for the nearest timestamp within PostGres. And then we want to use the currentTimeStamp everytime
    // after to search for the nearest timestamp in Postgres to that.
    // NOTE: The property of the customStreamItemObject below is hardcoded.
